...
type ParsedEntities<T extends SchemaType> = {
    items: StandardizedQueryResult<T>;
...
export class ToriiGrpcClient {
    ...
    async getEntities<T extends SchemaType = SchemaType>(
        query: Query
    ): Promise<ParsedEntities<T>> {
        const request = createRetrieveEntitiesRequest(query);
        const response =
            await this.client.worldClient.retrieveEntities(request).response;
        const entities = mapEntitiesResponse(response);

        // Parse entities and add to result
    ...
    async getAllEntities<T extends SchemaType = SchemaType>(
        limit: number,
        cursor?: string | null
    ): Promise<ParsedEntities<T>> {
        const query: Query = {
        ...
        return this.getEntities<T>(query);
    }

    async getEventMessages<T extends SchemaType = SchemaType>(
        query: Query
    ): Promise<ParsedEntities<T>> {
            ...
            onMessage: (response: SubscribeEntityResponse) => {
                if (response.entity) {
                    ...
                    for (const e of parsedEntities) {
                        callback(e, response.subscription_id);
                    }
                }
            },
        });
