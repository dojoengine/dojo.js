...
export class PredeployedAccountsConnector extends StarknetInjectedWallet {
    constructor(
        private readonly options: WithAccount<PredeployedAccountsConnectorOptions>
    ) {
        super(options.account.wallet);
    }
    ...
    async chainId(): Promise<bigint> {
        const chainId = await this.options.account.provider.getChainId();
        ...
        switch (call.type) {
            ...
            case "wallet_requestChainId":
                ...
                return shortString.encodeShortString(
                    await this.account.provider.getChainId()
                );
