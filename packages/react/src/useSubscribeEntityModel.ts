...
export function useSubscribeEntityModel<S extends Schema>(client: Client, component: Component<S, Metadata, undefined>, keys: any[]) {
    ...
    const setModelValue = async () => {
        try {
            setComponent(
                ...
                parseComponent(await client.getModelValue(componentName as string, keys_to_strings)) as any
            );
        } catch (error) {
