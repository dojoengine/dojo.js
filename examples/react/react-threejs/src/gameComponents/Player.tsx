...
export const Player = (props: any) => {
    ...
    const [hoveredTile, setHoveredTile] = useState<Direction | undefined>(undefined)
    const [startPosition, setStartPosition] = useState<THREE.Vector3 | undefined>();
    const [targetPosition, setTargetPosition] = useState<THREE.Vector3 | undefined>();
    ...
    const blueCellsAroundPlayer = [
        { direction: Direction.Up, position: new THREE.Vector3((vec.y - 1) * size, -size + 0.1, vec.x * size) },
        { direction: Direction.Down, position: new THREE.Vector3((vec.y + 1) * size, -size + 0.1, vec.x * size) },
        { direction: Direction.Right, position: new THREE.Vector3(vec.y * size, -size + 0.1, (vec.x + 1) * size) },
        { direction: Direction.Left, position: new THREE.Vector3(vec.y * size, -size + 0.1, (vec.x - 1) * size) }
    ]
    // Progress ref
    ...
    useFrame((_, delta) => {
        ...
        if (lerpProgress.current < 1) {
            ...
            if (coneRef.current) {
                coneRef.current.position.lerpVectors(startPosition, targetPosition, lerpProgress.current);
            }
    ...
    useEffect(() => {
        ...
        const newTargetPosition = new THREE.Vector3(vec.y * size, 0, vec.x * size)
        ...
        setStartPosition(new THREE.Vector3(prevVec.y * size, 0, prevVec.x * size));
    ...
    return (
        <>
            ...
            />
            {
                ...
                !moving && isLocalPlayer &&
                blueCellsAroundPlayer.map((cellInfo, k: number) => {
                    return (
                        <mesh
                            key={k}
                            receiveShadow
                            onClick={() => handleTileClick(cellInfo.direction)}
                            position={cellInfo.position}
                            geometry={squareGeometry}
                            material={
                                new THREE.MeshPhongMaterial({
                                    color: hoveredTile === cellInfo.direction ? 'lightblue' : 'blue',
                                })
                            }
                            onPointerEnter={(e) => {
